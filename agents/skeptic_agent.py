...
Be skeptical but constructive. Help us make a better decision by stress-testing it."""
    ...
    def run(self, state: AgentState) -> AgentState:
        """
        Challenge the hiring decision with critical analysis using LLM.
        
        Args:
            state: Current agent state
            
        Returns:
            Updated state with skeptical analysis
        """
        ...
        if self.llm:
            # LLM-powered natural language critique
            critique = self._build_critique_llm(candidate, job, scores, validation)
        else:
    ...
    def _build_critique_llm(self, candidate, job, scores, validation) -> str:
...
"""
        ...
        if missing_skills:
            ...
            critique += f"   ⚠️  Missing {len(missing_skills)} required skills: {', '.join(missing_skills)}\n"
        ...
        if candidate.experience_years < job.min_experience_years:
            ...
            critique += f"   ⚠️  {gap:.1f} years below minimum requirement\n"
            ...
            critique += f"   ⚠️  Significantly overqualified ({candidate.experience_years} years for {job.min_experience_years} year role)\n"
        ...
        if validation and not validation['detailed_checks']['budget']['strictly_compliant']:
            ...
            critique += f"   ⚠️  {budget_check['status'].upper()}\n"
        ...
        if overall < 75:
            ...
            critique += f"   ⚠️  Overall score: {overall:.0f}/100 (below 'Hire' threshold of 75)\n"
        ...
        if candidate.technical_interview_score and candidate.technical_interview_score < 65:
            ...
            critique += f"   ⚠️  Technical interview: {candidate.technical_interview_score:.0f}/100\n"
