...
Be persuasive but honest. Don't fabricate qualifications."""
    ...
    async def run(self, state: AgentState) -> AgentState:
        """Make the case FOR hiring the candidate."""
        candidate = state.candidate
        job = state.job
        scores = state.scores

        if self.llm:
            advocacy = await self._build_advocacy_llm(candidate, job, scores)
        else:
        ...
        logger.info("Advocate complete for candidate=%s", candidate.name)
        return state

    async def _build_advocacy_llm(self, candidate, job, scores) -> str:
...
TASK: Make a compelling, persuasive case FOR hiring this candidate. Highlight strengths, frame any gaps as growth opportunities, and explain why they're a strong fit. Be optimistic but honest."""
        ...
        advocacy = await self.llm.generate(
            prompt=context,
...
"""
        ...
        try:
            ...
            logger.debug("Memory helper not available for advocate agent")
