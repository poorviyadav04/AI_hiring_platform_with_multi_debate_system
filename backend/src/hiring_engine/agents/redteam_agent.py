...
Be rigorous but constructive. The goal is to make decisions STRONGER, not to reject all candidates."""
    ...
    async def run(self, state: AgentState) -> AgentState:
        ...
        if self.llm:
            analysis = await self._build_analysis_llm(
        ...
        logger.info("RedTeam complete: %d challenges found for candidate=%s",
                     len(self._extract_challenges(analysis)), candidate.name)
        return state

    async def _build_analysis_llm(self, candidate, job, scores, evaluator_msg, advocate_msg, skeptic_msg, moderator_msg) -> str:
...
"""
        ...
        if bias_check:
            challenges.append(f"BIAS ALERT: {bias_check}")
        ...
        if fairness:
            challenges.append(f"FAIRNESS CONCERN: {fairness}")
        ...
        if len(challenges) == 0:
            ...
            analysis += "DECISION CONDITIONAL - Minor concerns identified, proceed with caution\n"
        else:
    ...
    def _sensitivity_test(self, candidate, job, scores) -> str:
        ...
        if analysis['counterfactuals']:
            ...
            if 60 <= current_score < 65 and top_cf['overall_impact'] >= 5:
                result += "   WARNING: Minor change could flip to conditional hire\n"
            elif 65 <= current_score < 75 and top_cf['overall_impact'] >= 10:
                result += "   WARNING: Addressable gap could flip to hire\n"
        else:
    ...
    def _extract_challenges(self, analysis: str) -> List[str]:
        ...
        markers = ['BIAS ALERT', 'BOUNDARY CASE', 'INCONSISTENCY', 'EDGE CASE', 'FAIRNESS CONCERN']
        challenges = []
